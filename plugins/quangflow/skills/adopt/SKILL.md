---
name: adopt
description: "Use when onboarding an existing codebase into QuangFlow — scans project, scaffolds structure, generates reviewed CONTEXT.md"
---

You are now entering the Project Adoption command: `/quangflow:adopt`.
...
`/quangflow:adopt` is the entry point for teams bringing an existing project under QuangFlow governance.
Unlike `/quangflow:0-init` (which starts fresh), adopt performs a thorough codebase scan and infers
...
Differences from `/quangflow:0-init`:
...
- Routes to `/quangflow:1-brainstorm` (new feature) or `/quangflow:5-maintain` (maintenance) based on stated goal

## Arguments

```
/quangflow:adopt       — Interactive, no arguments required
```
...
1. Remove the DRAFT marker from CONTEXT.md:
   Remove: `> **STATUS: DRAFT** — Awaiting user approval via /quangflow:adopt review gate.`

2. Remove DRAFT markers from all `.memory/` files written during scaffolding:
   In each `.memory/` file, remove the line: `<!-- Generated by /quangflow:adopt — STATUS: DRAFT -->`
   Replace it with: `<!-- Generated by /quangflow:adopt — approved {ISO-8601 date} -->`
...
- Adopted on {ISO-8601 date} via /quangflow:adopt
...
1. /quangflow:1-brainstorm  — Discover requirements, edge cases, milestones, and team composition
                       ↳ Best if you have a new feature or enhancement to build on this project

2. /quangflow:5-maintain    — Post-ship systematic debugging, structured log scan, hotfix
                       ↳ Best if you want to fix bugs or maintain existing functionality

{if adoption_goal == "maintenance"}
Recommended for your goal:

1. /quangflow:5-maintain    — Post-ship systematic debugging, structured log scan, hotfix
                       ↳ Recommended based on your maintenance goal

2. /quangflow:1-brainstorm  — Discover requirements, edge cases, milestones, and team composition
...
- `new_feature` → suggest `/quangflow:1-brainstorm` (option 1)
- `maintenance` → suggest `/quangflow:5-maintain` (option 1)
- `both` → suggest `/quangflow:1-brainstorm` (option 1, note both are equally valid)
